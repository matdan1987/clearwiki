<?php
// views/demo_footer.php - Footer für Demo-Seite mit statischen Daten
// Diese Datei wird von views/demo_home.php geladen.
global $_SETTINGS, $current_lang, $is_demo_context; // is_demo_context hinzufügen
$is_demo_context = $is_demo_context ?? false; // Sicherstellen, dass die Variable immer gesetzt ist

// Dynamische Footer-Links abrufen (diese kommen aus der DB, deren URLs müssen dort korrekt sein)
        ...
        <div class="container mx-auto px-4 py-8">
                    ...
                    <ul class="space-y-2 text-sm text-gray-400">
                        <?php foreach ($footer_links['wiki'] as $link): ?>
                            <?php if ($link['is_active']): ?>
                                <li><a href="/<?= htmlspecialchars($current_lang ?? DEFAULT_LANG) ?><?= $is_demo_context ? '/demo' : '' ?><?= htmlspecialchars($link['url']) ?>" class="hover:text-orange-500"><?= __($link['text_key']) ?></a></li>
                            <?php endif; ?>
                        <?php endforeach; ?>
                    </ul>
                </div>
            ...
            <div class="border-t border-gray-800 mt-8 pt-8 text-center">
                ...
                <p class="text-xs text-gray-500 mt-2">
                    ...
                    Server Response: <span class="text-blue-500" id="server-response-time">23ms</span>
                </p>
            </div>
        </div>
    </footer>
